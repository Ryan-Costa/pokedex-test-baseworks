...
const respectiveColor = (pokemon: PokemonDetails) => {
  return pokemon.types
    .map((type) => typeColors[type.type.name as PokemonTypeColors])
    .slice(0, 1)[0];
};

const abbreviatedBaseStats = ["HP", "ATK", "DEF", "SATK", "SDEF", "SPD"];
</script>

<template>
  <div
    v-if="pokemon"
    class="relative w-full h-full min-h-screen flex flex-col p-1 md:px-20 lg:px-32"
    :style="{ backgroundColor: respectiveColor(pokemon) }"
  >
    <img
      class="absolute w-[70%] h-[45%] right-0 opacity-10 z-0"
    ...
    <div class="relative w-full h-full flex flex-col z-20 mt-6">
      <div v-if="pokemon" class="flex w-full flex-col items-center">
        ...
        >
          ...
          <img
            v-if="pokemon && pokemon.sprites.other.dream_world.front_default"
            class="w-48 md:w-64 z-20"
          ...
          <router-link
            :disabled="pokemon && pokemon.id >= MAX_POKEMON_ID"
            :to="`/pokemon/${pokemon.id + 1}`"
          >
            <span
              :class="`pi pi-angle-right text-2xl md:text-4xl ${
                pokemon.id >= MAX_POKEMON_ID ? 'opacity-20' : ''
              }`"
            ></span>
          </router-link>
        </div>
        ...
        <div
          class="flex flex-col items-center bg-background shadow-inside-custom -mt-10 w-full rounded-lg bottom-0 z-10 px-5 pt-14 pb-16"
        >
          ...
          <div class="flex w-full my-4">
            <div
              class="w-full flex flex-col gap-1 items-center justify-center border-r-2 border-grayscaleBackground"
            >
            ...
            <div class="w-full flex flex-col gap-1 items-center justify-center">
              <div class="flex flex-col">
                <span
                  ...
                  class="text-darkPrimary text-center capitalize text-sm"
          ...
          <p class="text-darkPrimary text-md text-center py-4">
            {{ pokemon.flavorText }}
          </p>
          ...
          <div class="w-full flex flex-col justify-start mt-4">
            <div class="flex items-center space-x-4">
              <div
                class="flex flex-col space-y-2 bg-white border-r-2 border-grayscaleBackground"
              >
                <p
              ...
              <div class="flex flex-col space-y-2">
                <p
                  ...
                  :key="index"
                  class="text-darkPrimary text-left"
                >
              ...
              <div class="w-full flex flex-col space-y-6">
                ...
                >
                  <div
                    ...
                    :style="{
                      width: `${(stats.base_stat / 200) * 100}%`,
                      backgroundColor: respectiveColor(pokemon),
                    }"
                  ></div>
                </div>
              </div>
